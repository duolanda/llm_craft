...
The standard ruleset no longer produces `soldier`; barracks production starts with `rifleman` and also supports `rocket_soldier`. A completed war factory unlocks the T2 `scout_car` and `light_tank`; a completed tech center unlocks the T3 `heavy_tank` and `artillery` in war factories. The `soldier` type remains readable and targetable so older Match Records can still be replayed and inspected.
