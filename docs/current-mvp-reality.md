...
`attack_move_unit` 是暴露给 agent 的无目标区域推进命令：战斗单位会向目标点移动并在到达前自动攻击路上的敌方单位。到达目标点后该命令结束，不会持续自动攻击后续靠近或新生产的敌方单位。它只用于没有明确 `targetId` 时穿越危险区域或试探接敌；拆 HQ、拆 barracks、点杀敌军应使用 `attack`。
...
新增护甲类型与伤害类型交互机制。所有单位和建筑都有护甲类型，部分单位有伤害类型。`HQ` 和 `barracks` 都是 `concrete` 护甲，不按 `heavy` 结算。

**伤害/护甲交互矩阵:**
| 攻击类型 | 对轻甲 | 对重甲 | 对混凝土 |
|---------|--------|--------|----------|
| 穿刺 (piercing) | 100% | 50% | 35% |
| 爆炸 (explosive) | 50% | 150% | 60% |
| 无属性 | 100% | 100% | 100% |

**单位属性表:**
| 单位 | HP | 攻击 | 射程 | 攻击冷却 | 护甲 | 伤害类型 | 造价 |
|------|-----|------|------|----------|------|---------|------|
| Worker | 50 | 0 | 0 | 0 tick | 轻甲 | - | 50 |
| Soldier | 80 | 12 | 1 | 2 ticks | 轻甲 | 穿刺 | 80 |
| Tank | 220 | 28 | 1 | 2 ticks | 重甲 | 无 | 300 |
| Demolisher | 40 | 25 | 3 | 4 ticks | 轻甲 | 爆炸 | 160 |

**建筑属性表:**
| 建筑 | HP | 护甲 | 造价 |
|------|-----|------|------|
| HQ | 1000 | 混凝土 | 0 |
| Barracks | 300 | 混凝土 | 120 |

**定位:** Tank 昂贵，不能在开局第一座 barracks 后立即量产，主要负责中后期攻城和抗线；Demolisher 射程 3、攻速慢，主要远程压制 heavy 单位，不再强拆 HQ；Soldier 是基础输出和早期主力。Barracks 可生产 soldier/tank/demolisher。
