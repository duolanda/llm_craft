...
| 地图尺寸 | `21 x 21` |
| Tick 间隔 | `500ms` |
| AI 默认思考频率 | 每 `5 tick` 一次 |
| 开局资源 | 每方 `200 credits` |
| 开局配置 | 每方 `1 HQ + 2 Worker` |
| 胜利条件 | 摧毁敌方 `HQ` |

#### 单位数值

| 单位 | HP | 速度 | 攻击力 | 攻击范围 | 造价 | 备注 |
|------|----|------|--------|----------|------|------|
| `worker` | `50` | `1` | `0` | `0` | `50 credits` | 可建造 `barracks`，可采集资源 |
| `soldier` | `100` | `1` | `15` | `1` | `80 credits` | 可攻击单位和建筑 |

#### 建筑数值

| 建筑 | HP | 造价 | 可生产/功能 | 备注 |
|------|----|------|-------------|------|
| `hq` | `1000` | `0` | 生产 `worker` | 开局主基地，也是交付资源目标 |
| `barracks` | `300` | `120 credits` | 生产 `soldier` | 由 `worker` 瞬间建造完成 |

#### 经济数值

| 项目 | 当前数值 |
|------|----------|
| Worker 单次采集速率 | 每 tick `10 credits` |
| Worker 携带上限 | `100 credits` |
| HQ 交付范围 | `1` |
...
| 项目 | 当前数值 |
|------|----------|
| Worker / Soldier 每 tick 最大移动距离 | `1` 格 |
