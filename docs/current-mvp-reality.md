...
`call_to_arms` 是暴露给 agent 的一次性紧急技能：每方整局只能成功使用一次，只影响激活瞬间仍存活的 worker。受影响 worker 会在短时间内获得近战攻击能力；代价是进入 `40 tick` 的采矿效率降低状态。该技能不会自动触发，是否使用完全由模型自己决策。
...
单位有一层自卫保底：当有攻击力的单位被敌方单位攻击后，如果它仍然存活、处于 idle/hold、没有正在执行的移动路径，且攻击者在自身射程内，会自动还击攻击者。这个机制只处理单位自身被打后的反击；HQ / barracks 不会触发周围士兵自动护卫，也不会替代 `attack` / `attack_move_unit` 的主动作战决策。
...
- `call_to_arms` 每方一次：当前存活 worker 临时获得攻击能力，并承受 40 tick 采矿衰减
