...
- **现状**：单位移动给人的观感仍是一卡一卡的，部分单位在移动过程中还会出现位置抖动；目前仍未定位。
...
- **实战证据与处理**：根据 `match-2026-07-29T15-07-50-390Z-13343a75` 定位到 OBB 坦克贴近 HQ 且无法原地转向的模拟层死锁。坦克现在可沿车身轴脱困，A* 中间格也不再要求精确压中；同处互相阻塞的 worker 可随之继续路径。剩余的视觉卡顿和位置抖动尚未定位，继续作为独立活跃问题跟踪。
