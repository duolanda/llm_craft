...
- **新增边界**：持续 8 tick 无路线进展后可以触发带瞬时拥堵代价的延迟重规划，但每 tick 最多处理 4 个单位，不会恢复成同 tick 的无界 A* 重试。
...
- **现状**：单位/建筑模型、弹丸和基本碰撞已存在；步兵死亡已与载具/建筑爆炸分流，保留原兵种模型执行程序化倒地、短暂停留和淡出。步兵仍缺基于骨骼的完整行走、射击和死亡动画。
...
- **实战证据**：`match-2026-08-22T12-47-33-267Z-cbb2c6d9` 在 tick 1088 后双方基地的 worker、步兵和车辆大量保留有效路径与移动命令，但长期停在下一路径格附近。静态地图仍可达；旧实现即使继续推进 180 tick，从 player_2 录像压缩出的 15 个战斗单位也只有 5 个绕出重工区域，12 个 worker 只有 9 个完成过交付。
- **根因与处理**：全局 A* 原本永久忽略移动单位，`blockedTicks` 只增加局部候选角度，所以单位不会放弃被人群占据的近路。现在另行跟踪整条路线的实际进展；连续 8 tick 无改善后，以静止单位、移动单位和近期失败路段构造有限软代价，重算可以更远绕行的路线；重规划按拥堵时间排序并限制为每 tick 4 个单位。共享矿点另有终点语义冲突：第二个 worker 会被投影到相邻落点，采集却只认矿格中心。worker 现可从矿格边缘交互，保留每矿最多 2 人的分配规则。
- **验证**：新增不依赖 15 MB 原录像的紧凑回归场景，保留 tick 1088 的重工/兵营/机枪塔建筑墙、连续坐标与朝向、12 个 worker 的共享矿路以及 15 个西向战斗单位。命令经 `Game` 边界进入完整 `SimulationCore`；120 tick 内 15 个战斗单位全部绕出重工，12 个 worker 全部产生采集或交付进展，至少 10 个完成交付，终态无碰撞重叠，两次独立运行结果完全一致。另一压力测试确认完全围堵时也只在窗口边界重规划，阻挡者移除后可恢复移动。
