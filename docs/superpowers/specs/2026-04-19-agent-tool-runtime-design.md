...
当前地图很小，且没有战争迷雾，因此 v1 不需要 `get_match_overview` 或 `get_map_region` 这种拆分视图。

只需要一个 `get_map_state`，直接返回当前全图可见信息。
...
单位和建筑应直接带上阵营归属字段，避免再拆成单独的敌方查询工具。

它的作用是给模型一张当前完整战场图，模型再根据需要决定是否继续调用 `get_my_units`、`get_my_state` 或别的工具做更细的决策。
...
这个工具保留，是为了避免模型每次要控制我方单位时都从全图结果里手动过滤一次。

建议返回：

- 我方全部单位列表
- 每个单位的当前位置、状态、血量、攻击范围、载货状态
- 当前单位是否已有激活计划

它的作用是作为“可直接操作我方单位”的便利入口，而不是替代全图视图。
