...
describe("AISandbox", () => {
  ...
  it("should expose attackMoveTo and serialize the sustained attack command", async () => {
    const game = new Game();
    const aiState = AIStatePackageBuilder.build("player_1", game.getState(), game);
    const sandbox = new AISandbox("player_1");

    const result = await sandbox.executeCode(
      "me.workers[0].attackMoveTo({ x: enemyBuildings[0].x, y: enemyBuildings[0].y }, ['hq', 'worker']);",
      aiState
    );
  ...
  it("should expose harvestLoop and serialize the optional resource target", async () => {
    const game = new Game();
    const aiState = AIStatePackageBuilder.build("player_1", game.getState(), game);
    const sandbox = new AISandbox("player_1");
